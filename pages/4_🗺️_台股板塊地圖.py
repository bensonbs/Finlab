...
def create_treemap_data(start, end, item, clip=None):
    ...
    try:
        temp = close_data.loc[start]
    except:
        st.warning('🚨 無開始日資料')
        return None

    try:
        temp = close_data.loc[end]
    except:
        st.warning('🚨 無結束日資料')
        return None
...
clip=1000
#@title 台股漲跌與市值板塊圖
start= st.date_input("開始日",datetime.datetime.strptime(prev_day(2), '%Y-%m-%d')+ datetime.timedelta(hours = 8)).strftime('%Y-%m-%d')
end = st.date_input("結束日",datetime.datetime.strptime(prev_day(1), '%Y-%m-%d')+ datetime.timedelta(hours = 8)).strftime('%Y-%m-%d')
ar = st.empty()
ar.info('🖋 努力繪製中')
if start < end:
